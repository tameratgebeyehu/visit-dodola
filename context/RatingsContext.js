...
export const RatingsProvider = ({ children }) => {
  ...
  useEffect(() => {
    const loadRatings = async () => {
      try {
        ...
        console.error('Failed to load ratings from SecureStore', e);
      }
    };
    loadRatings();
  }, []);
  ...
  const saveRatings = async (newRatings) => {
    try {
      await SecureStore.setItemAsync(STORAGE_KEY, JSON.stringify(newRatings));
    } catch (e) {
      console.error('Failed to save ratings to SecureStore', e);
    }
  };
